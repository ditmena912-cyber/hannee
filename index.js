...
// Hàm định dạng thời gian DÀNH RIÊNG CHO BẢO TRÌ: Loại bỏ giây, chỉ giữ lại phút
...
// Hàm tính toán thời gian cho Boss (VẪN GIỮ NGUYÊN GIÂY)
...
// Gửi tin nhắn thông báo Boss (Giữ nguyên giây)
    ...
    embeds: [
      {
        ...
        color: 16724736,
        fields: [
          { name: "👹 Tên Boss", value: `**${info.bossName}**`, inline: true },           { name: "🗺️ Bản đồ", value: `**${info.mapName}**`, inline: true },
          { name: "🌐 Máy chủ", value: `**${info.serverName}**`, inline: true },           { name: "⏰ Thời gian ra", value: `\`${info.timeStr}\``, inline: false },           { name: "⚡ Hỗ trợ (+7 phút 30 giây)", value: `\`${info.supportSpawnTime}\``, inline: false },           { name: "📞 Hỗ trợ Zalo", value: "Lỗi thông báo liên hệ Zalo **0366 517 900** (Han Đây)", inline: false }         ],         footer: { text: "⚔️ Hệ Thống Báo Boss 15 Sao ⚔️" }       }     ]   };    try {     await axios.post(webhookUrl, payload);     console.log(`[Discord Send] Đã gửi thông báo Boss: ${info.bossName}`);      if (isTeamLeader(info.bossName)) {       await sendFinalSummaryWebhook(webhookUrl, info.timeStr, info.mapName);     }   } catch (err) {     console.error("[Discord Error] Lỗi khi gửi webhook Boss:", err.message);   } }  // Gửi tin nhắn tổng kết mốc Số 4 (Giữ nguyên giây) async function sendFinalSummaryWebhook(webhookUrl, currentTime, currentMap) {   const baseTime = lastNumberFourItem ? lastNumberFourItem.timeStr : currentTime;   const baseMap = lastNumberFourItem ? lastNumberFourItem.mapName : currentMap;   const labelNote = lastNumberFourItem ? "🟢 Số 4 ra lúc" : "⚠️ Mốc tham chiếu (Chưa thấy Số 4)";    const estimatedNext = calculateNextTime(baseTime, 15, 0);        const estimatedSupport = calculateNextTime(baseTime, 7, 30);      const summaryPayload = {     username: "millims15",     avatar_url: "https://i.imgur.com/4M34hi2.png",     embeds: [       {         title: "📊 THỐNG KÊ THỜI GIAN TỪ MỐC SỐ 4 📊",         color: 3447003,         fields: [           { name: labelNote, value: `**${baseTime}** tại khu vực **${baseMap}**`, inline: false },
...
// Gửi tin nhắn thông báo Bảo trì (ĐÃ LOẠI BỎ GIÂY, chỉ hiển thị đến phút)
async function sendMaintenanceWebhook(item) {
  ...
  const formattedTime = formatMaintenanceTime(rawTime); // Lọc bỏ giây
...
async function fetchBossApi() {
  try {
    // 1. Quét danh mục BOSS cho máy chủ 15 sao
    ...
    // 2. Quét ĐÚNG danh mục BẢO TRÌ cho máy chủ 15 sao dựa trên ảnh giao diện web
    ...
    if (Array.isArray(maintList)) {
      for (const item of maintList) {
        const maintId = `maint_${item.id || item.time}`;
